...
#     GSM.initStartMode()                 # НАСТРОЙКА РЕЖИМА ЗАПУСКА СКРИПТОВ
    ...
    while(1):
        ...
        if(context != '1'):
            DEBUG.send('Start activation GPRS context')
            GSM.activateContext()
            DEBUG.send('GPRS context activated')
            resetWatchdog()
        ...
        if(socket not in ['1', '2', '3']):   # Socket closed
            ...
            try:
                ...
                continue
            DEBUG.send('Socket #1 opened (data)')
            resetWatchdog()
        ...
        if((CONFIG.get('REG_SERVER') == '1') and (DATA_SOCKET == 1) and (DATA_AUTH == 0)):
            ...
            if((len(data) > 0)): # and (data.find(AUTH_REQUEST) != -1)):
                ...
                DEBUG.send('Authorization socket #1 failed')
                continue
            DATA_AUTH = 1
            DEBUG.send('Authorization socket #1 complete')
            resetWatchdog()
        ...
        if(CONFIG.get('AUTH_TYPE') != '1'):
            if(CONFIG.get('DEBUG_TCP') == '1'):
                ...
                if(socket not in ['1', '2', '3']):   # Socket closed
                    ...
                    try:
                        ...
                        DEBUG.send(e)
                    DEBUG.send('Socket #2 opened (log)')
                    resetWatchdog()
            ...
            if((CONFIG.get('REG_LOG_SERVER') == '1') and (LOG_SOCKET == 1) and (LOG_AUTH == 0)):
                ...
                if((len(data) > 0)): # and (data.find(AUTH_REQUEST) != -1)):
                    ...
                    DEBUG.send('Authorization socket #2 failed')
                LOG_AUTH = 1
                DEBUG.send('Authorization socket #2 complete')
                resetWatchdog()
